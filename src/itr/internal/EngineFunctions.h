...
inline auto Actor_GetDetectionValue = (INT32(__thiscall *)(Actor *, Actor *))0x8A8230;
...
struct DetectionData {
  ...
  INT32 detectionValue;    // 0x08 - the threshold value (-100..500)
  float locX;              // 0x0C
  float locY;              // 0x10
  float locZ;              // 0x14
...
inline void *GetSettingDataPtr(void *setting) { return setting ? Setting_GetDataPtr(setting) : nullptr; }

inline float *GetSettingFloatPtr(void *setting) { return static_cast<float *>(GetSettingDataPtr(setting)); }

inline UINT8 *GetSettingUInt8Ptr(void *setting) { return static_cast<UINT8 *>(GetSettingDataPtr(setting)); }
...
inline void *GameHeapAlloc(UINT32 size) { return GameHeap_Allocate(g_gameHeap, size); }

inline void GameHeapFree(void *ptr) {
  if (ptr)
    GameHeap_Free(g_gameHeap, ptr);
}
