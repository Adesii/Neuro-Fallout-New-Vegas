...
public:
  FakeDirectInputDevice(IDirectInputDevice8 *device, DWORD type) : m_device(device), m_deviceType(type), m_refs(1) {
    //
  }

  HRESULT _stdcall QueryInterface(REFIID riid, LPVOID *ppvObj) { return m_device->QueryInterface(riid, ppvObj); }
...
public:
  /*** Constructor ***/
  FakeDirectInput(IDirectInput8 *obj) : m_realDInput(obj), m_refs(1) {}
  ...
  HRESULT _stdcall CreateDevice(REFGUID typeGuid, IDirectInputDevice8A **device, IUnknown *unused) {
    if (typeGuid != GUID_SysKeyboard && typeGuid != GUID_SysMouse) {
      ...
      *device =
          new FakeDirectInputDevice(dev, (typeGuid == GUID_SysKeyboard) ? kDeviceType_Keyboard : kDeviceType_Mouse);
  ...
  HRESULT _stdcall EnumDevices(DWORD a, LPDIENUMDEVICESCALLBACKA b, void *c, DWORD d) {
    return m_realDInput->EnumDevices(a, b, c, d);
  }
  ...
  HRESULT _stdcall EnumDevicesBySemantics(LPCSTR a, LPDIACTIONFORMATA b, LPDIENUMDEVICESBYSEMANTICSCBA c, void *d,
                                          DWORD e) {
    return m_realDInput->EnumDevicesBySemantics(a, b, c, d, e);
  }
  HRESULT _stdcall ConfigureDevices(LPDICONFIGUREDEVICESCALLBACK a, LPDICONFIGUREDEVICESPARAMSA b, DWORD c, void *d) {
    return m_realDInput->ConfigureDevices(a, b, c, d);
  }
...
static HRESULT _stdcall Hook_DirectInput8Create_Execute(HINSTANCE instance, DWORD version, REFIID iid, void *out,
                                                        IUnknown *outer) {
...
void Hook_DirectInput8Create_Init() {
  ...
  HookUtils::SafeWrite32(thunkAddress, (DWORD)Hook_DirectInput8Create_Execute);
}

DIHookControl::DIHookControl() { memset(&m_keys, 0, sizeof(m_keys)); }
...
HRESULT DIHookControl::ProcessBufferedData(IDirectInputDevice8 *device, DWORD dataSize, DIDEVICEOBJECTDATA *outData,
                                           DWORD *outDataLen, DWORD flags) {
  ...
  HRESULT hr = device->GetDeviceData(dataSize, outData, (LPDWORD)numRealEvents, flags);
...
FramerateTracker::FramerateTracker()
    : m_lastTime(0), m_lastFrameLength(0), m_frameTimeHistoryIdx(0), m_frameTimeHistoryPrimed(false),
