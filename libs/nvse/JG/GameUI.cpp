...
#if 1
static const uint32_t s_RaceSexMenu__UpdatePlayerHead =
    0x007B25A0; // End of RaceSexMenu::Func003.case0, call containing QueuedHead::Init (3rd before jmp)
static uint8_t *g_bUpdatePlayerModel =
    (uint8_t *)0x011C5CB4; // this is set to true when player confirms change of race in RaceSexMenu -
                           // IF requires change of skeleton - and back to false when model updated#elif EDITOR
#else
#error
#endif

InterfaceManager *InterfaceManager::GetSingleton(void) { return *(InterfaceManager **)0x011D8A80; }
...
void MapMenu::PlayHolotape(BGSNote *note, bool playStartStopSound) {
  ...
  if (note->type == BGSNote::kSound) {
    BSSoundHandle sound = BSWin32Audio::GetSingleton()->GetSoundHandleByFormID(
        note->voice->GetFormID(), BSAudioManager::kAudioFlags_2D | BSAudioManager::kAudioFlags_100);
  ...
  if (isHolotapeVoicePlaying) {
    if (playStartStopSound) {
      BSSoundHandle sound = BSWin32Audio::GetSingleton()->GetSoundHandleByEditorName(
          ...
          BSAudioManager::kAudioFlags_100 | BSAudioManager::kAudioFlags_SystemSound | BSAudioManager::kAudioFlags_2D);
